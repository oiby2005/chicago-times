...
const rightPanelArticles: RightMainPanelArticle[] = [
  {
    ...
    imageUrl: "https://images.unsplash.com/photo-1519741497674-611481863552?fm=webp&fit=crop&w=600&q=80",
...
export const RightMainPanelSection: React.FC = () => {
      ...
      if (stored) {
        ...
        if (rmPosts.length > 0) {
          const formatted: RightMainPanelArticle[] = rmPosts.slice(0, 2).map((p: any) => ({
            ...
            title: p.title,
            slug: p.slug || p.id,
            imageUrl: p.thumbnail || "https://images.unsplash.com/photo-1519741497674-611481863552?fm=webp&fit=crop&w=600&q=80",
            imageAlt: p.title,
          }));
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between font-sans select-none space-y-3">
      {articles.map((article) => (
        <article key={article.id} className="flex flex-col justify-between flex-1">
          <Link href={`/article/${article.slug}`} className="block relative aspect-[16/8] w-full overflow-hidden bg-gray-100 mb-1.5 group">
            <img
